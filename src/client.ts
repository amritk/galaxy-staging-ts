...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env["OFFICIAL_GALAXY_TESTING_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["OFFICIAL_GALAXY_TESTING_LOG"] or 'warn' if it isn't set.
   */
...
export class DemoAPIScalarGalaxy {
   ...
   * @param {string | null | undefined} [opts.webhookSecret=process.env["OFFICIAL_GALAXY_TESTING_WEBHOOK_SECRET"] ?? null]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env["OFFICIAL_GALAXY_TESTING_BASE_URL"] ?? https://galaxy.scalar.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('OFFICIAL_GALAXY_TESTING_BASE_URL'),
    ...
    webhookSecret = readEnv('OFFICIAL_GALAXY_TESTING_WEBHOOK_SECRET') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    if (baseURLOverridden && options.environment)
      throw new Errors.DemoAPIScalarGalaxyError(
        'Ambiguous URL; The `baseURL` option (or OFFICIAL_GALAXY_TESTING_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null',
      );
    ...
    this.logLevel =
      ...
      parseLogLevel(
        readEnv('OFFICIAL_GALAXY_TESTING_LOG'),
        'process.env["OFFICIAL_GALAXY_TESTING_LOG"]',
        this,
      ) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('OFFICIAL_GALAXY_TESTING_CUSTOM_HEADERS');
    if (customHeadersEnv) {
