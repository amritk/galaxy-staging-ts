...
| `baseURL` | `string \| null` | `process.env["OFFICIAL_GALAXY_TESTING_BASE_URL"]` | Override the default API base URL. Pass `null` when selecting a configured environment. |
...
| `logLevel` | `"off" \| "error" \| "warn" \| "info" \| "debug" \| null` | `process.env["OFFICIAL_GALAXY_TESTING_LOG"]` | Controls request and retry debug logging. |
